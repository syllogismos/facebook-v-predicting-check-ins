...
def train_single_grid_cell(m, n, state):
    ...
    if len(data.shape) == 1:
        ...
        data_feature = data_feature.reshape(1, len(data_feature))
    ...
    if len(data_combined) == 1:
        top_t_train_preds = np.hstack((data_combined[:, 0].reshape(-1, 1), [top_t]*len(data_combined)))
    ...
    mask = np.array(map(lambda x: state['grid'].M[m][n][x] > state['threshold'], data_combined[:, -1]))
    ...
    if len(masked_data) < 10:
        ...
        # np.savetxt(file_name, top_t_train_preds, fmt = '%s', delimiter = ',')
        return None, None, None, top_t, top_t_train_preds
    X, x_transformer = trans_x(masked_data[:, 1:-1])
    Y, y_transformer = trans_y(masked_data[:, -1])
    ...
    if len(Y) == 0:
        ...
        X_orig, x_transformer = trans_x(data_combined[:, 1:-1], x_transformer)
        ...
        top_t_train_preds = np.hstack((data_combined[:, 0].reshape(-1, 1), top_t_train_preds))
        top_t_train_preds = top_t_train_preds.astype(int)
        # file_name = folder + '_'.join(['top_t_preds', str(m), str(n)]) + '.csv'
        # np.savetxt(file_name, top_t_train_preds, fmt = '%s', delimiter = ',')
...
class XGB_Model(SklearnModel):
    ...
    def train_and_predict_parallel(self, submission_file, feature_prefix = 'grid_200_50_20_5_def_params_th7_n100', upload_to_s3 = False):
        ...
        for i in range(len(cv_combined)):
            m, n = get_grids_of_a_point((cv_combined[i][1], cv_combined[i][2]), self.grid)[0]
            cv_grid_wise_data[m][n].append(cv_combined[i])
        ...
        state['feature_submission_name'] = feature_submission_name
